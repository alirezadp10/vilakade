...
function Hero() {
  ...
  if (cityState.length > 0) {
    showCityesFilter = citiesList.filter((item) => item.name.match(cityState));
  }
    ...
    <HeroContainer className="mt-3">
      <div className="relative w-full md:h-full h-fit">
        ...
        >
          ...
          >
            <HiOutlineLocationMarker size={30} className="text-gray-400" />
                ...
                {showCityesFilter.map((item, index) =>
                  index < 5 ? (
                    <li
                      ...
                      onClick={() => handleSearchFromList(item.name)}
                    >
                      {item.name}
